...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("purchaseNotificationForm")
    .addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (response.ok) {
          alert("Purchase notification submitted successfully!");
          this.reset();
          // Reset book dropdown and quantity
          populateBookDropdown();
        } else {
          alert(
            `Error: ${result.error || "Failed to submit purchase notification"}`
          );
        }
      } catch (error) {
...
async function populateBookDropdown() {
  ...
  try {
    ...
    if (!response.ok) response = await fetch("/api/books");
    if (!response.ok) throw new Error("Failed to fetch books");
    const data = await response.json();
    const books = Array.isArray(data)
      ? data
      : data.listings || data.books || [];
    ...
    books.forEach((book) => {
      ...
      option.textContent = `${book.title} by ${book.author} (Stock: ${
        book.quantity || 1
      })`;
...
async function populateBookDropdown() {
  const select = document.getElementById("bookSelect");
  try {
    // Try both endpoints just in case, fallback logic
    let resp = await fetch("/api/listings?limit=1000");
    if (!resp.ok) resp = await fetch("/api/books?limit=1000");
    if (!resp.ok) throw new Error("Failed to load books");
    const books = await resp.json();
    // Support both array and .listings structure
    const bookArray = Array.isArray(books)
      ? books
      : books.listings || books.books || [];
    bookArray.forEach((book, i) => {
      const title = book.title || book.name;
      const author = book.author || book.writer;
      if (!title || !author) return; // skip incomplete
      const option = document.createElement("option");
      option.value = JSON.stringify({
        id: book._id || book.id,
        title,
      ...
      option.textContent = `${title} by ${author}`;
      select.appendChild(option);
    });
  } catch (err) {
    select.innerHTML = '<option value="">Could not load books</option>';
...
document.addEventListener("change", function (e) {
  if (e.target && e.target.id === "bookSelect") {
    if (e.target.value) {
      ...
      document.getElementById("bookAuthor").value = obj.author;
    } else {
...
function validateForm(formData) {
  ...
  if (!bookTitle) {
    showAlert("Please enter the book title", "error");
    document.getElementById("bookTitle").focus();
    return false;
  }

  if (!bookAuthor) {
    showAlert("Please enter the author name", "error");
...
function handleSuccessResponse(result) {
  const message = `
        Purchase report submitted successfully!
...
function handleErrorResponse(result, statusCode) {
  ...
  if (statusCode === 404) {
    errorMessage = `Book not found: ${
      result.message ||
      "Please verify the book title and author match exactly as listed in the inventory"
    }`;
  } else if (statusCode === 400) {
    if (result.error === "Book is out of stock") {
      errorMessage = `${
        result.bookTitle
      } is currently out of stock (Available: ${result.availableStock || 0})`;
 ...
 * Log initialization
 */
console.log("Purchase notification form initialized with MongoDB integration");
